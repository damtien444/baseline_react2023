...
def rc_recurrent(
    x_start, pred, batch_first=True
):  # residual connection => offsets modeling
    # x_start -> [batch_size, ...]
    # pred -> [seq_length, batch_size, ...] | [batch_size, seq_length, ...]
    if batch_first:
...
class BasicMLP(nn.Module):
    def __init__(
        self, input_dim, output_dim, hidden_dims=[], dropout=0.5, non_linearities="relu"
    ):
        ...
        hidden_dims = hidden_dims + [
            output_dim,
        ]  # output dim is treated as the last hidden dim
...
class AutoencoderRNN_VAE_v2(nn.Module):
    def __init__(self, cfg):
        ...
        assert (
            self.seq_len % self.window_size == 0
        ...
        self.d_rnn = RNN(
            self.emotion_dim + self.hidden_dim, self.hidden_dim, cell_type=self.rnn_type
        )
    ...
    def _decode(self, h_y):
        ...
        for i in range(self.window_size):
            y_p = (
                torch.zeros((h_y.shape[0], self.emotion_dim), device=h_y.device)
                if i == 0
                else y_i
            )
    ...
    def encode_all(self, emotion_seq):
        ...
        emotion_seq = emotion_seq.reshape(
            batch_size * (seq_len // self.window_size), self.window_size, -1
        )
    ...
    def encode(self, emotion_seq):
        ...
        if seq_len > self.window_size:
            ...
            selected_emotion_seq = emotion_seq[
                :,
                selected_window
                * self.window_size : (selected_window + 1)
                * self.window_size,
                :,
            ]
    ...
    def forward(self, listener_emotion=None, listener_3dmm=None, **kwargs):
        ...
        emotion = listener_emotion.reshape(
            batch_size * (frame_num // self.window_size), self.window_size, -1
        )
        # as a result, we have the (seq_len // window_length) windows for each batch element, sequentially

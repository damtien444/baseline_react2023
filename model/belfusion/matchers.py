...
class BaseLatentModel(nn.Module):
    def __init__(
        ...
        assert os.path.exists(embedder_emotion_path) and os.path.exists(
            configpath
        ), f"Missing checkpoint/config file for auxiliary model: '{embedder_emotion_path}'"
        ...
        assert (
            "statistics" in checkpoint or emb_preprocessing.lower() == "none"
        ), "Model statistics are not available in its checkpoint. Can't apply embeddings preprocessing."
        self.embed_emotion_stats = (
            checkpoint["statistics"] if "statistics" in checkpoint else None
        )
    ...
    def deepcopy(self):
        assert (
            self.init_params is not None
        ), "Cannot deepcopy LatentUNetMatcher if init_params is None."
    ...
    def preprocess(self, emb):
        ...
        if "standardize" in self.emb_preprocessing:
            ...
            raise NotImplementedError(
                f"Error on the embedding preprocessing value: '{self.emb_preprocessing}'"
            )
...
class LatentMLPMatcher(BaseLatentModel):
    def __init__(self, cfg):
        super(LatentMLPMatcher, self).__init__(
            cfg,
        ...
        self.emb_length = (
            cfg.emb_length
        )  # TODO multiply by 2 if using speaker + listener embeddings
        self.window_size = cfg.window_size
        assert (
            self.window_size == self.embed_emotion.window_size
        ), "window_size must be the same as the emotion embedder"
    ...
    def forward_offline(self, speaker_emotion=None, listener_emotion=None, **kwargs):
        ...
        if is_training:  # sample random subwindow for each batch element
            ...
            window_start = torch.randint(
                0, self.window_size, (1,), device=speaker_emotion.device
            )
            window_end = window_start + self.window_size
            # target to be predicted (and forward diffused)
            x_start = listener_emotion[:, window_start:window_end]
            t, weights = self.schedule_sampler.sample(
                batch_size, speaker_emotion.device
            )

            output = self.diffusion.denoise(
                self,
                self.model,
                x_start,
                t,
            ...
            listener_emotion = listener_emotion.reshape(
                diff_batch, self.window_size, -1
            )

            output = [
                output
                for output in self.diffusion.ddim_sample_loop_progressive(
                    self,
                    self.model,
                    diff_batch,
                    model_kwargs={
            ...
            output = {
                key: output[key].reshape(batch_size, -1, *output[key].shape[2:])
                for key in output
            }
            return output
    ...
    def forward_online(self, speaker_emotion=None, listener_emotion=None, **kwargs):
        ...
        first_window_prediction = torch.zeros_like(
            listener_emotion[:, : self.window_size]
        )  # naive first window prediction
        if is_training:
            ...
            return self.forward_offline(
                speaker_emotion_shifted, listener_emotion_shifted, **kwargs
            )
            ...
            speaker_emotion_shifted = torch.cat(
                [
                    torch.zeros_like(speaker_emotion[:, : self.window_size]),
                    speaker_emotion[:, : -self.window_size],
                ],
                dim=1,
            )

            return self.forward_offline(
                speaker_emotion_shifted, listener_emotion, **kwargs
            )
...
if __name__ == "__main__":
    ...
    timesteps = torch.tensor(
        np.array(np.random.randint(0, 1000, batch_size), dtype=int), device=device
    )

    emb_listener = torch.zeros(
        (batch_size, emb_length), device=device, dtype=dtype
    ).contiguous()
    emb_speaker = torch.zeros(
        (batch_size, emb_length), device=device, dtype=dtype
    ).contiguous()

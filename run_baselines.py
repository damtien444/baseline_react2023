...
def parse_args():
    ...
    parser.add_argument(
        "--dataset-path",
        default="/home/luocheng/Datasets/S-L",
        type=str,
        help="dataset path",
    )
    parser.add_argument(
...
def get_baseline(
    cfg, baseline, num_pred=10, speaker_emotion=None, listener_emotion=None
):
    ...
    if baseline == "MeanSeq" or baseline == "MeanFr":
        ...
        if training_mean is None or training_mean_single is None:
            train_loader = get_dataloader(
                cfg, "train", load_emotion_s=True, load_emotion_l=True
            )
            ...
            training_mean = all_tr_emotion[None, None, ...].repeat(
                batch_size, num_pred, 1, 1
            )
            training_mean_single = single_tr_emotion[None, None, ...].repeat(
                batch_size, num_pred, training_mean.shape[2], 1
            )

        return (
            training_mean[:batch_size]
            if baseline == "MeanSeq"
            else training_mean_single[:batch_size]
        )
...
def val(cfg):
    ...
    for i, baseline in enumerate(baselines):
        ...
        assert (
            all_speaker_emotion.shape[0] == all_pred_listener_emotion.shape[0]
        ), "Number of predictions and number of speaker emotions must match ({} vs. {})".format(
            all_pred_listener_emotion.shape[0], all_speaker_emotion.shape[0]
        )
        ...
        FRC = compute_FRC_mp(
            cfg,
            all_pred_listener_emotion,
            all_listener_gt_emotion,
            p=p,
            val_test=cfg.split,
        )
        ...
        print(
            "[{}/{}] Baseline: {}, FRC: {:.5f} | FRD: {:.5f} | S-MSE: {:.5f} | FRVar: {:.5f} | FRDvs: {:.5f} | TLCC: {:.5f}".format(
                i + 1, len(baselines), baseline, FRC, FRD, smse, FRVar, FRDvs, TLCC
            )
        )

        print(
            "Latex-friendly --> B\\_{} & {:.2f} & {:.2f} & {:.4f} & {:.4f} & {:.4f} & - & {:.2f} \\\\".format(
                baseline, FRC, FRD, smse, FRVar, FRDvs, TLCC
            )
        )

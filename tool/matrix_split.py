...
def parse_arg():
    ...
    parser.add_argument(
        "--partition", default="val", type=str, help="dataset partition"
    )
...
data_path = pd.read_csv(
    os.path.join(root_path, "data_indices.csv"), header=None, delimiter=","
)
...
val_path = pd.read_csv(
    os.path.join(root_path, str(args.partition) + ".csv"), header=None, delimiter=","
)
...
for index, row in val_path.iterrows():
    ...
    items = row[1].split("/")
    if len(items) > 4:
        path_val_item = os.path.join(
            items[0].upper(), items[2] + "_" + items[1], items[4]
        )
    else:
...
for index, item in enumerate(index_list):
    for j, item_j in enumerate(index_list):
        ...
        new_matrix[index + l_m, j + l_m] = all_matrix[
            item + all_matrix_w // 2, item_j + all_matrix_w // 2
        ]


np.save(
    os.path.join(root_path, "neighbour_emotion_" + str(args.partition) + ".npy"),
    new_matrix,
)

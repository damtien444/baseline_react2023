...
def cut_udiva_videos(in_base_path, out_base_path, session="animal"):
    with open("./udiva_list.csv", newline="") as f:
        ...
        video_to_indices = {}
        for row in reader:
            video_to_indices.setdefault((row["subject"], row["topic"]), set()).add(
                int(row["index"])
            )
        ...
        if topic == session:
            for part in ["FC1", "FC2"]:
                in_path = os.path.join(
                    os.path.join(in_base_path, subject), f"{part}_{session_mark}.mp4"
                )
...
if __name__ == "__main__":
    ...
    extract_recola_videos(
        "/home/batubal/Desktop/recola_videos", "recola_video_files", option="video"
    )

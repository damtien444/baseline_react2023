...
from dataset import ReactionDataset, get_dataloader
...
def parse_arg():
    ...
    parser.add_argument(
        "--clip-length", default=256, type=int, help="len of video clip"
    )
    parser.add_argument(
        "--window-size",
        default=8,
    ...
    for batch_idx, (
        ...
        if torch.cuda.is_available():
            speaker_video_clip, speaker_audio_clip, listener_emotion, listener_3dmm = (
                speaker_video_clip.cuda(),
                speaker_audio_clip.cuda(),
                listener_emotion.cuda(),
                listener_3dmm.cuda(),
            )
        ...
        wandb.log(
            {
                ...
                "train_div_loss": d_loss.data.item(),
            }
        )
                ...
                if (batch_idx % 50) == 0:
                    for bs in range(B):
                        render.rendering(
                            val_path,
                            "e{}_b{}_ind{}".format(
                                str(epoch + 1), str(batch_idx + 1), str(bs + 1)
                            ),
            ...
            wandb.log(
                {
                    ...
                    "val_rec_loss": rec_loss.data.item(),
                    "val_kld_loss": kld_loss.data.item(),
                }
            )
...
def main(args):
    ...
    if torch.cuda.is_available():
        model = model.cuda()
        render = Render("cuda:0")
    else:
        render = Render()

    for epoch in range(start_epoch, args.epochs):
        train_loss, rec_loss, kld_loss, div_loss = train(
            args, model, train_loader, optimizer, criterion
        )
        print(
            "Epoch:  {}   train_loss: {:.5f}   train_rec_loss: {:.5f}  train_kld_loss: {:.5f} train_div_loss: {:.5f}".format(
                epoch + 1, train_loss, rec_loss, kld_loss, div_loss
            )
        )
        if (epoch + 1) % args.val_epoch == 0:
            val_loss, rec_loss, kld_loss = val(
                args, model, val_loader, criterion, render, epoch
            )
            print(
                "Epoch:  {}   val_loss: {:.5f}   val_rec_loss: {:.5f}  val_kld_loss: {:.5f} ".format(
                    epoch + 1, val_loss, rec_loss, kld_loss
                )
            )
...
if __name__ == "__main__":
    ...
    # for id in range(292, len(dataset)):
    #     # print(dataset.data_list[id])
    #     try:
    #         sample = dataset[id]
    #         a = sample[0]
    #         print("success", id)

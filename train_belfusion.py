...
def evaluate(cfg, pred_list_em, speaker_em, listener_em, epoch):
    ...
    """
    assert (
        listener_em.shape[0] == speaker_em.shape[0]
    ), "speaker and listener emotion must have the same shape"
    assert (
        listener_em.shape[0] == pred_list_em.shape[0]
    ), "predictions and listener emotion must have the same shape"
    ...
    metrics = {
        ...
        "FRVar": compute_FRVar(
            pred_list_em
        ),  # FRVar (1) --> intra-variance (among all frames in a prediction),
        "FRDiv": compute_s_mse(
            pred_list_em
        ),  # FRDiv (2) --> inter-variance (among all predictions for the same speaker),
        "FRDvs": compute_FRDvs(
            pred_list_em
        ),  # FRDvs (3) --> diversity among reactions generated from different speaker behaviours
        # OTHER METRICS
        # FRRea (realism)
...
def train(cfg, model, train_loader, optimizer, criterion, device):
    ...
    for batch_idx, (
        ...
        prediction = model(
            speaker_emotion=s_emotion, listener_emotion=l_emotion, listener_3dmm=l_3dmm
        )  # returns dictionary with "prediction" and "target"
...
def validate(cfg, model, val_loader, criterion, device, epoch, mode):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (
            ...
            s_emotion_, l_emotion_, l_3dmm_ = (
                s_emotion.to(device),
                l_emotion.to(device),
                l_3dmm.to(device),
            )
            if mode == "predict":  # then predict num_preds
                s_emotion_ = s_emotion_.repeat_interleave(
                    num_preds, dim=0
                )  # [num_preds*batch_size, seq_length, 25]
                l_emotion_ = l_emotion_.repeat_interleave(
                    num_preds, dim=0
                )  # [num_preds*batch_size, seq_length, 25]
                l_3dmm_ = l_3dmm_.repeat_interleave(
                    num_preds, dim=0
                )  # [num_preds*batch_size, seq_length, 25]

            prediction = model(
                speaker_emotion=s_emotion_,
                listener_emotion=l_emotion_,
                listener_3dmm=l_3dmm_,
            )  # [num_preds*batch_size, seq_length, 25]
            assert "target" in prediction, "target must be in prediction"
            ...
            if mode == "predict":
                ...
                prediction = {
                    key: prediction[key].view(-1, num_preds, *prediction[key].shape[1:])
        ...
        if mode == "predict":
            ...
            )  # [num_preds*dataset_size, num_preds, seq_length, 25]
            speaker_emotion = torch.cat(
                speaker_emotions, dim=0
            )  # [num_preds*dataset_size, seq_length, 25]
            listener_emotion = torch.cat(
                listener_emotions, dim=0
            )  # [num_preds*dataset_size, seq_length, 25]
    ...
    return {
        "val_" + key: losses_meters[key].avg for key in losses_meters
    }, metrics_results
...
def compute_statistics(config, model, data_loader, device):
    ...
    with torch.no_grad():
        for batch_idx, (
            ...
            speaker_emotion, listener_emotion = speaker_emotion.to(
                device
            ), listener_emotion.to(device)
            prediction = model.encode_all(
                speaker_emotion
            )  # [num_windows * batch_size, features_size]
...
def main():
    ...
    print(
        "Model {} : params: {:4f}M".format(
            cfg.arch.type, sum(p.numel() for p in model.parameters()) / 1000000.0
        )
    )
    ...
    assert mode in [
        "autoencode",
        "predict",
    ], "mode must be either autoencode or predict"
    ...
    for epoch in range(start_epoch, cfg.trainer.epochs):
        ...
        # =================== VALIDATION ===================
        if (
            cfg.trainer.val_period > 0 and (epoch + 1) % cfg.trainer.val_period == 0
        ) or epoch == start_epoch:
            val_losses, val_metrics = validate(
                cfg, model, valid_loader, criterion, device, epoch, mode
            )
        ...
        if cfg.trainer.save_period > 0 and (epoch + 1) % cfg.trainer.save_period == 0:
            ...
            torch.save(
                checkpoint,
                os.path.join(cfg.trainer.out_dir, "checkpoint_{}.pth".format(epoch)),
            )
            # remove older
            older_path = os.path.join(
                cfg.trainer.out_dir,
                "checkpoint_{}.pth".format(epoch - cfg.trainer.save_period),
            )
    ...
    if mode == "autoencode":
        ...
        cfg.resume = os.path.join(
            cfg.trainer.out_dir, f"checkpoint_{last_epoch_stored}.pth"
        )
